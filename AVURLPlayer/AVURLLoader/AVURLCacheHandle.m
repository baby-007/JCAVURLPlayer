...
#import "NSString+AVURLAdd.h"

@interface AVURLCacheHandle ()
...
@implementation AVURLCacheHandle
...
- (NSString *)tempFilePath {
    if (!_tempFilePath) {
        NSString *tempFilePath = [AVURLCacheHandle tempFilePath:self.tempFileName];
...
- (void)cacheTempFileWithFileName:(NSString *)fileName {
    ...
    NSString *cacheFilePath = [AVURLCacheHandle cacheFilePath:[NSString stringWithFormat:@"%@.mp4", self.fileName]];
    [fileManager removeItemAtPath:cacheFilePath error:nil];
    [fileManager copyItemAtPath:self.tempFilePath toPath:cacheFilePath error:nil];
...
- (void)cacheTempFileWithURL:(NSURL *)URL {
    ...
    self.fileName = [URL.absoluteString au_md5];
...
+ (NSString *)cacheFileExistsWithURL:(NSURL *)URL {
    ...
    NSString *cacheFilePath = [AVURLCacheHandle cacheFilePath:[NSString stringWithFormat:@"%@.mp4", fileName]];
...
+ (NSString *)cacheFileDirectory {
    ...
    NSString *cacheDirectory = [arrPaths.firstObject stringByAppendingPathComponent:@"AVURLVideoCache"];
    NSFileManager *manager = [NSFileManager defaultManager];
    NSError *error;
...
+ (NSString *)cacheFilePath:(NSString *)fileName {
    NSString *tempPath = [AVURLCacheHandle cacheFileDirectory];
...
+ (NSString *)tempFileDirectory {
    ...
    NSString *cacheDirectory = [tempPath stringByAppendingPathComponent:@"AVURLVideoTemp"];
    NSFileManager *manager = [NSFileManager defaultManager];
    NSError *error;
    if (![manager fileExistsAtPath:cacheDirectory]) {
        [manager createDirectoryAtPath:cacheDirectory withIntermediateDirectories:YES attributes:nil error:&error];
...
+ (BOOL)clearAllCacheFile {
    ...
    NSError *error;
    NSFileManager *manager = [NSFileManager defaultManager];
    BOOL result = [manager removeItemAtPath:[AVURLCacheHandle cacheFileDirectory] error:&error];
...
+ (BOOL)clearAllTempFile {
    NSError *error;
    NSFileManager *manager = [NSFileManager defaultManager];
    BOOL result = [manager removeItemAtPath:[AVURLCacheHandle tempFileDirectory] error:&error];
